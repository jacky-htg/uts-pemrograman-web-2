...
try {
  ...
  $condition  = isset($where['search']) ? 'WHERE nama like ? OR nim LIKE ? OR program_studi LIKE ?' : '';
  ...
  $sort = [
    'field' => isset($_GET['sort_field']) && in_array($_GET['sort_field'], ['id', 'nama', 'nim', 'program_studi']) ? $_GET['sort_field'] : 'id',
...
function listData($db, $condition, $where, $sort, $offset, $limit) {
  $query = "SELECT * FROM mahasiswa $condition";
